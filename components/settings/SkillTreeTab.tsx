  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in-up w-full max-w-[1600px] mx-auto px-1 sm:px-0 transition-colors duration-300">
      <div className="w-full max-w-lg mx-auto mb-2">
        <div className="grid grid-cols-4 gap-1 bg-white dark:bg-white/5 p-1.5 rounded-[18px] md:rounded-[22px] border border-gray-200 dark:border-transparent shadow-sm dark:shadow-inner transition-colors">
          {(['재배', '채광', '해양', '사냥'] as Profession[]).map((tab) => (
            <button
              ...
              className={`flex items-center justify-center py-2 md:py-2.5 rounded-[14px] md:rounded-[16px] font-bold transition-all text-[11px] sm:text-xs shadow-sm ${
                ...
                : 'text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/5 shadow-none'
              }`}
            >
              {tab}
            </button>
      ...
      <div className="flex flex-col lg:flex-row gap-6 w-full justify-center items-start transition-colors">
        <div className="w-full lg:flex-1 bg-white dark:bg-[#0a0a0a] border border-gray-200 dark:border-transparent rounded-[2rem] p-5 sm:p-8 shadow-sm dark:shadow-2xl overflow-hidden flex flex-col order-1 lg:max-w-[1500px] transition-colors">
          ...
          {profTab === '재배' && (
            <div className="mb-6 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-transparent text-emerald-700 dark:text-emerald-400 px-4 py-3 rounded-2xl text-[10px] sm:text-xs font-bold text-center leading-relaxed break-keep transition-colors">
              [씨앗은 덤이야], [왕 크니까 왕 좋아], [오늘도 풍년이다!] 스킬은<br className="sm:hidden" />
              요리 효율 분석의 <span className="text-gray-900 dark:text-white border-b border-emerald-300 dark:border-transparent">베이스 원가 기댓값</span>에 자동 반영됩니다.
            </div>
          )}
        ...
        <div className="w-full lg:w-80 flex flex-col gap-6 flex-shrink-0 order-2 transition-colors">
          <div className="bg-white dark:bg-[#0a0a0a] dark:bg-gradient-to-br dark:from-amber-900/20 dark:to-black border border-gray-200 dark:border-transparent rounded-[2rem] p-6 shadow-sm dark:shadow-2xl transition-colors">
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={i} className={`flex justify-between items-center ${i < 2 ? 'border-b border-gray-100 dark:border-transparent pb-4' : ''} transition-colors`}>
          ...
          <div className="bg-white dark:bg-[#0a0a0a] border border-gray-200 dark:border-transparent rounded-[2rem] p-6 shadow-sm dark:shadow-2xl flex-1 min-h-[250px] lg:min-h-[400px] flex flex-col transition-colors">
