      ...
      <div className="flex flex-wrap gap-2 border-b border-gray-200 dark:border-white/5 pb-4 transition-colors">
        {(Object.keys(BOX_DATA) as Array<keyof typeof BOX_DATA>).map(key => (
          <button
            ...
            className={`px-5 py-2.5 text-sm font-semibold rounded-lg transition-colors flex-1 md:flex-none ${
              selectedBoxKey === key
                ? 'bg-blue-100 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 border border-blue-300 dark:border-blue-500/30 shadow-sm'
        ...
        <div className="ml-auto flex items-center gap-3 w-full md:w-auto mt-2 md:mt-0 justify-end">
          <button 
            onClick={handleReset} 
