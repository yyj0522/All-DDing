...
const BIGHEAD_REWARDS: Reward[] = [
  ...
  { id: 'mid_head', name: '중두 치장 획득권', prob: 2, image: `${STORAGE_BASE_URL}/capsule2/feather_pen.png`, amount: 1, grade: 'legendary' },
    ...
    <div className="w-full space-y-6 relative transition-colors duration-300">
      ...
      <div className="flex flex-wrap gap-4 items-center justify-between border-b border-gray-200 dark:border-white/10 pb-4 transition-colors">
        <div className="flex flex-wrap gap-2">
          ...
          <button onClick={() => setMode('snipe')} className={`px-4 py-2 text-sm font-bold rounded-lg transition-colors ${mode === 'snipe' ? 'bg-rose-100 dark:bg-rose-500/20 text-rose-600 dark:text-rose-400 border border-rose-200 dark:border-rose-500/30' : 'text-gray-500 dark:text-gray-500 hover:text-gray-900 dark:hover:text-gray-300'}`}>특정 아이템 저격</button>
        </div>
        ...
        <div className="flex items-center gap-3">
          {mode === 'normal' && (
            <div className="flex items-center gap-2 mr-2">
              ...
              <label htmlFor="anim2" className="text-sm font-bold text-gray-600 dark:text-gray-300 cursor-pointer select-none hover:text-gray-900 dark:hover:text-white transition-colors">룰렛 켜기</label>
            </div>
          )}
            ...
            >
              {strip.map((item, i) => (
                <div key={i} className="w-[100px] h-[92px] flex-shrink-0 flex flex-col items-center justify-center">
                  ...
                  <span className="text-[10px] mt-2 text-center text-gray-600 dark:text-gray-300 font-bold line-clamp-1">{item.name}</span>
                </div>
              ))}
            </div>
      ...
      {mode === 'test' && (
        <div className="space-y-6 animate-fade-in">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-gray-50 dark:bg-white/5 p-6 rounded-2xl border border-gray-200 dark:border-white/10 gap-4 transition-colors">
            ...
            <button onClick={handleMassTest} className="bg-blue-600 hover:bg-blue-500 text-white font-bold px-6 py-3 rounded-xl transition-all shadow-md dark:shadow-[0_0_15px_rgba(37,99,235,0.3)] whitespace-nowrap">
              즉시 1만번 돌리기
            </button>
          </div>
      ...
      {mode === 'snipe' && (
        <div className="space-y-6 animate-fade-in">
          <div className="bg-white dark:bg-[#0a0a0a] border border-rose-200 dark:border-rose-500/20 rounded-2xl p-6 sm:p-8 shadow-sm dark:shadow-[0_0_30px_rgba(244,63,94,0.05)] transition-colors">
            <h3 className="text-xl font-black text-rose-600 dark:text-rose-400 mb-2 transition-colors">특정 아이템 저격 모드</h3>
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-6 transition-colors">원하는 아이템이 나올 때까지 뒷단에서 캡슐을 무한으로 연속 개봉합니다.</p>
            
            <div className="flex flex-col sm:flex-row gap-4 mb-8">
              <select 
                value={snipeTargetId} 
                onChange={(e) => setSnipeTargetId(e.target.value)}
                className="flex-1 bg-gray-50 dark:bg-black border border-gray-300 dark:border-white/10 text-gray-900 dark:text-white text-sm rounded-xl px-4 py-3 focus:outline-none focus:border-rose-400 dark:focus:border-rose-500 cursor-pointer transition-colors"
              >
              ...
              <button 
                ...
                className="bg-rose-600 hover:bg-rose-500 text-white font-bold px-8 py-3 rounded-xl disabled:opacity-50 disabled:cursor-not-allowed shadow-md dark:shadow-[0_0_15px_rgba(225,29,72,0.3)] transition-all whitespace-nowrap"
              >
            ...
            <div ref={containerRef} className="relative w-full">
              {snipeResult && (
                <div className="bg-gray-50 dark:bg-black/50 border border-gray-200 dark:border-white/10 rounded-xl p-8 flex flex-col items-center justify-center text-center animate-fade-in-up relative z-10 backdrop-blur-sm transition-colors">
                  ...
                  <div className="mt-4 text-gray-600 dark:text-gray-300 font-medium flex flex-col md:flex-row items-center justify-center gap-2 transition-colors">
                    해당 아이템을 뽑기 위해 캡슐을
                    <span className="bg-gray-200 dark:bg-white/10 px-3 py-1 rounded-lg transition-colors">총 <span className="text-rose-600 dark:text-rose-400 font-black text-xl transition-colors">{snipeResult.attempts.toLocaleString()}</span>회</span>
                    <span className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-500/20 px-3 py-1 rounded-lg flex items-center gap-1.5 transition-colors">
                      <img src={`${STORAGE_BASE_URL}/crystal.png`} className="w-5 h-5 object-contain" alt="크리스탈" />
                      <span className="text-blue-600 dark:text-blue-400 font-black text-xl transition-colors">{snipeResult.cost.toLocaleString()}</span> 크리스탈 사용
                    </span>
                  </div>
                </div>
              )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 dark:bg-black/60 backdrop-blur-sm animate-fade-in" onClick={() => setShowProbModal(false)}>
          <div className="bg-white dark:bg-[#111] border border-gray-200 dark:border-white/10 rounded-2xl p-6 max-w-md w-full shadow-2xl transition-colors" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-lg font-black text-fuchsia-600 dark:text-fuchsia-400 transition-colors">대두 치장 캡슐 확률표</h3>
