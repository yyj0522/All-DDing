...
export default function BigHeadCapsuleSimulator() {
  ...
  return (
    <div className="w-full space-y-6 relative transition-colors duration-300">
      ...
      <div className="bg-white dark:bg-[#0e0e12] border border-gray-200 dark:border-white/10 rounded-[2rem] p-5 md:p-6 shadow-lg dark:shadow-black/30 transition-colors max-w-5xl mx-auto mt-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-5">
          <div>
            <h3 className="text-lg md:text-xl font-black text-gray-900 dark:text-white transition-colors">대두 치장 캡슐 확률표</h3>
            <p className="text-xs md:text-sm text-gray-500 dark:text-gray-400 mt-1 max-w-xl transition-colors">항목별 획득 확률을 아래에서 빠르게 확인하세요. 스크롤만 내려도 언제든 보입니다.</p>
          </div>
        </div>
          ...
          {BIGHEAD_REWARDS.map((item, idx) => (
            <div key={idx} className="group relative hover:z-50 flex items-center gap-3 bg-gray-50 dark:bg-white/5 p-4 rounded-3xl border border-gray-100 dark:border-white/5 hover:bg-gray-100 dark:hover:bg-white/10 transition-colors cursor-default">
              <div className="relative w-12 h-12 shrink-0 rounded-2xl bg-white dark:bg-[#12131a] flex items-center justify-center shadow-sm dark:shadow-black/20 overflow-hidden">
                <Image src={item.image} alt="I" fill unoptimized className="drop-shadow-sm dark:drop-shadow-none object-contain" />
              </div>
              <div className="min-w-0">
                <div className="text-sm md:text-base font-bold text-gray-900 dark:text-white truncate">{item.name} {item.amount > 1 ? `x${item.amount}` : ''}</div>
                <div className="text-[11px] md:text-xs text-gray-500 dark:text-gray-400 mt-1">획득 확률</div>
              </div>
              <div className="text-sm font-black text-fuchsia-600 dark:text-fuchsia-300 bg-fuchsia-100 dark:bg-fuchsia-500/20 px-3 py-1 rounded-full transition-colors whitespace-nowrap">
                {item.prob.toFixed(4)}%
              </div>
            </div>
          ))}
