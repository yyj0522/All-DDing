    ...
    <div className="w-full space-y-6 relative">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-black/40 border border-yellow-500/20 rounded-2xl p-5 shadow-lg gap-4">
        <div className="flex items-center gap-4">
          <img src={`${STORAGE_BASE_URL}/dino/gold_chest.png`} alt="공룡 보급품" className="w-16 h-16 object-contain drop-shadow-lg" />
          <div>
          ...
          <div ref={containerRef} className="bg-[#0a0a0a] border border-white/10 rounded-2xl relative overflow-hidden h-[450px] flex flex-col items-center justify-center shadow-inner w-full">
            ...
            {!wonItem && (
              <div className="relative z-10 flex flex-col items-center justify-center h-full w-full">
                {openAnimState === 'idle' && (
                  <img src={`${STORAGE_BASE_URL}/dino/gold_chest.png`} alt="보급품 상자" className="w-32 h-32 object-contain drop-shadow-[0_10px_20px_rgba(0,0,0,0.8)] transition-transform duration-300 hover:scale-110" />
                )}
                      ...
                      <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-20">
                        {[0, 90, 180, 270].map((deg, i) => (
                          <div key={i} className="absolute flex justify-center w-0 h-0" style={{ transform: `rotate(${deg + 45}deg)` }}>
                            <div className="absolute bottom-0 w-[60px] h-[500px] bg-gradient-to-t from-transparent via-yellow-100 to-white rounded-t-full blur-[8px]" style={{ animation: `ray-shoot 0.8s ease-out forwards ${i * 0.25}s`, opacity: 0, transformOrigin: 'bottom center' }} />
                          </div>
                        ))}
                      </div>
                    )}
            ...
            <div className={`absolute inset-0 bg-[radial-gradient(ellipse_at_center,rgba(0,0,0,0.4)_0%,rgba(0,0,0,0.9)_100%)] z-30 flex flex-col items-center justify-center backdrop-blur-sm transition-opacity duration-300 ${!isSpinning && wonItem && openAnimState === 'idle' ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
              {wonItem && (
                <div className="flex flex-col items-center justify-center w-full h-full" style={{ animation: 'item-float-up 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards' }}>
                  <div style={{ animation: 'float-idle 3s ease-in-out infinite' }}>
                    <img src={wonItem.image} alt={wonItem.name} className="w-48 h-48 object-contain drop-shadow-[0_15px_25px_rgba(0,0,0,0.8)] rendering-pixelated" />
                  </div>
                    ...
                    return (
                      <tr key={reward.id} className="border-b border-white/5 last:border-0 hover:bg-white/5 transition-colors">
                        <td className="px-4 py-4 font-bold text-white flex items-center gap-3">
                          <div className="w-8 h-8 rounded border border-white/10 bg-black/50" style={{ backgroundImage: `url(${reward.image})`, backgroundSize: 'contain', backgroundPosition: 'center', backgroundRepeat: 'no-repeat', imageRendering: 'pixelated' }} />
                          {reward.name}
                        </td>
                        <td className="px-4 py-4 text-gray-400">{reward.prob.toFixed(4)}%</td>
                        <td className={`px-4 py-4 font-black ${isAccurate ? 'text-green-400' : 'text-red-400'}`}>{actualProb.toFixed(4)}%</td>
      ...
      {mode === 'snipe' && (
        <div className="space-y-6 animate-fade-in">
          <div className="bg-[#0a0a0a] border border-yellow-500/20 rounded-2xl p-6 sm:p-8 shadow-[0_0_30px_rgba(234,179,8,0.05)]">
            ...
            <div className="flex flex-col gap-4 mb-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2 max-h-[300px] overflow-y-auto custom-scrollbar p-1">
                {DINO_REWARDS.map(r => {
                  ...
                  return (
                    <div key={r.id} onClick={() => toggleSnipeTarget(r.id)} className={`cursor-pointer flex items-center gap-2 p-2 rounded-lg border transition-all select-none ${isSelected ? 'bg-yellow-500/20 border-yellow-500 shadow-[inset_0_0_10px_rgba(234,179,8,0.3)]' : 'bg-black/50 border-white/10 hover:bg-white/5'}`}>
                      <div className="w-8 h-8 rounded border border-white/10 bg-black/50 shrink-0" style={{ backgroundImage: `url(${r.image})`, backgroundSize: 'contain', backgroundPosition: 'center', backgroundRepeat: 'no-repeat', imageRendering: 'pixelated' }} />
            ...
            <div className="flex justify-center">
              <button onClick={handleSnipe} disabled={snipeTargetIds.length === 0 || isSniping} className="bg-gradient-to-r from-yellow-600 to-amber-500 hover:from-yellow-500 hover:to-amber-400 text-black font-black px-12 py-4 rounded-xl disabled:opacity-50 disabled:cursor-not-allowed shadow-[0_0_20px_rgba(245,158,11,0.4)] transition-all">
        ...
        <div className="space-y-6 animate-fade-in">
          <div className="bg-[#0a0a0a] border border-indigo-500/20 rounded-2xl p-6 sm:p-8 shadow-[0_0_30px_rgba(99,102,241,0.05)]">
            ...
            <div className="flex flex-col sm:flex-row gap-4 mb-8 items-end">
              ...
              <button onClick={handleBudgetSimulate} className="bg-indigo-600 hover:bg-indigo-500 text-white font-bold px-8 py-3.5 rounded-xl shadow-[0_0_15px_rgba(79,70,229,0.3)] transition-all whitespace-nowrap h-[52px]">일괄 개봉하기</button>
            </div>
            {budgetResult && (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                  {budgetResult.results.map(([reward, count]) => (
                    <div key={reward.id} className="flex flex-col items-center justify-center p-4 rounded-xl border bg-black/50 border-white/10">
                      <div className="w-12 h-12 rounded border border-white/10 bg-black/50 mb-3" style={{ backgroundImage: `url(${reward.image})`, backgroundSize: 'contain', backgroundPosition: 'center', backgroundRepeat: 'no-repeat', imageRendering: 'pixelated' }} />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-[60vh] overflow-y-auto custom-scrollbar pr-2">
              {DINO_REWARDS.map((item, idx) => (
                <div key={idx} className="flex items-center gap-3 bg-white/5 p-2 rounded-lg text-sm">
                  <div className="w-8 h-8 rounded border border-white/10 bg-black/50 shrink-0" style={{ backgroundImage: `url(${item.image})`, backgroundSize: 'contain', backgroundPosition: 'center', backgroundRepeat: 'no-repeat', imageRendering: 'pixelated' }} />
                  <span className="text-gray-200 flex-1 font-bold truncate">{item.name}</span>
                  <span className="text-white font-bold bg-yellow-500/20 text-yellow-400 px-2 py-1 rounded whitespace-nowrap">{item.prob.toFixed(4)}%</span>
                </div>
              ))}
            </div>
          </div>
