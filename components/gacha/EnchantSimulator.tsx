...
export default function EnchantSimulator() {
  ...
  return (
    <div className="w-full space-y-6 md:space-y-8 relative transition-colors duration-300">
      <div className="grid grid-cols-2 sm:flex sm:flex-wrap lg:flex-nowrap gap-3 md:gap-4 px-1">
        {ENCHANT_BOXES.map((box) => (
          ...
          >
            ...
            <span className={`text-[10px] md:text-xs font-bold text-center break-keep leading-tight ${activeBox?.id === box.id ? 'text-fuchsia-700 dark:text-white' : 'text-gray-700 dark:text-gray-300'}`}>{box.name}</span>
          </button>
        ))}
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between border-b border-gray-200 dark:border-white/10 pb-4 transition-colors">
        ...
        <div className="flex items-center justify-between w-full md:w-auto gap-4">
          ...
          <button onClick={() => setShowProbModal(true)} className="flex-1 md:flex-none px-4 py-2 bg-fuchsia-50 dark:bg-fuchsia-500/10 text-fuchsia-600 dark:text-fuchsia-400 text-xs md:text-sm font-bold border border-fuchsia-200 dark:border-fuchsia-500/30 rounded-lg hover:bg-fuchsia-100 dark:hover:bg-fuchsia-500/20 transition-colors">확률표</button>
        </div>
      </div>

      {mode === 'normal' && (
        <div className="w-full space-y-8 animate-fade-in">
          <div ref={containerRef} className="bg-gray-100 dark:bg-[#0a0a0a] border border-gray-200 dark:border-white/10 rounded-2xl relative overflow-hidden h-60 md:h-72 flex flex-col justify-center shadow-inner w-full transition-colors">
            <div className="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 flex gap-[20px] z-0 opacity-20 md:opacity-100">
              {Array.from({ length: 21 }).map((_, i) => (
                <div key={i} className={`w-[100px] h-[92px] rounded-xl flex-shrink-0 border transition-colors ${i === 10 ? 'bg-red-100 dark:bg-red-900/20 border-red-300 dark:border-red-500/50' : 'bg-white dark:bg-gray-900/50 border-gray-200 dark:border-white/5'}`}></div>
              ))}
            </div>
            ...
            <div ref={trackRef} className="absolute top-1/2 flex gap-[20px] z-10" style={{ left: '50%', transform: `translate(${offset}px, -50%)`, transition: 'none' }}>
              {strip.map((item, i) => (
                <div key={i} className="w-[100px] h-[92px] flex-shrink-0 flex flex-col items-center justify-center">
                  ...
                  <span className="text-[9px] md:text-[10px] mt-2 text-center text-gray-700 dark:text-gray-300 font-bold line-clamp-1 transition-colors">{item.name}</span>
                </div>
              ))}
            </div>

            <div className="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[116px] h-[108px] border-4 border-red-500 rounded-2xl z-20 shadow-sm dark:shadow-lg pointer-events-none transition-colors"></div>
            ...
            {!isSpinning && wonItem && (
              <div className="absolute inset-0 bg-white/90 dark:bg-black/90 z-30 flex flex-col items-center justify-center backdrop-blur-sm animate-fade-in p-4 text-center transition-colors">
                ...
                <h3 className="text-xl md:text-3xl font-black text-gray-900 dark:text-white mt-4 break-keep transition-colors">
                  {wonItem.id === 'piece' ? '인챈트북 조각!' : wonItem.name} {wonItem.amount > 1 ? `(x${wonItem.amount})` : ''}
                </h3>
              </div>
            )}
          </div>
          <div className="flex justify-center"><button onClick={handleOpen} disabled={isSpinning || !activeBox} className="w-full md:w-auto bg-fuchsia-600 hover:bg-fuchsia-500 text-white font-black text-lg md:text-xl px-20 py-4 md:py-5 rounded-xl shadow-md dark:shadow-lg transform active:scale-95 transition-all">{isSpinning ? '개봉 중...' : '1회 개봉하기'}</button></div>
        </div>
      )}

      {mode === 'test' && (
        <div className="space-y-6 animate-fade-in">
          <div className="flex flex-col sm:flex-row justify-between items-center bg-gray-50 dark:bg-white/5 p-6 rounded-2xl border border-gray-200 dark:border-white/10 gap-4 transition-colors">
            ...
            <button onClick={handleMassTest} className="w-full sm:w-auto bg-blue-600 hover:bg-blue-500 text-white font-bold px-6 py-3 rounded-xl shadow-sm dark:shadow-lg transition-all">즉시 1만번 실행</button>
          </div>
          {testCount > 0 && (
                  ...
                  {currentRewards.map(reward => {
                    ...
                    return (
                      <tr key={reward.id} className="border-b border-gray-200 dark:border-white/5 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                        ...
                        <td className={`px-4 py-4 font-black transition-colors ${Math.abs(reward.prob - actual) < 1.0 ? 'text-green-600 dark:text-green-400' : 'text-yellow-600 dark:text-yellow-400'}`}>{actual.toFixed(4)}%</td>
      ...
      {mode === 'snipe' && (
        <div className="space-y-6 animate-fade-in">
          <div className="bg-white dark:bg-[#0a0a0a] border border-rose-200 dark:border-rose-500/20 rounded-2xl p-6 md:p-8 shadow-sm dark:shadow-xl transition-colors">
            <h3 className="text-lg md:text-xl font-black text-rose-600 dark:text-rose-400 mb-4 transition-colors">아이템 저격 모드</h3>
            <div className="flex flex-col gap-4">
              <select value={snipeTargetId} onChange={(e) => setSnipeTargetId(e.target.value)} className="w-full bg-gray-50 dark:bg-black border border-gray-300 dark:border-white/10 text-gray-900 dark:text-white text-sm rounded-xl px-4 py-3 outline-none focus:border-rose-400 dark:focus:border-rose-500 transition-colors">
                <option value="">저격할 목표 아이템을 선택하세요</option>
                {currentRewards.map(r => (<option key={r.id} value={r.id}>{r.name} - 확률 {r.prob}%</option>))}
              </select>
              <button onClick={handleSnipe} disabled={!snipeTargetId || isSniping} className="w-full bg-rose-600 hover:bg-rose-500 text-white font-bold py-4 rounded-xl shadow-sm dark:shadow-lg transition-all">{isSniping ? '찾는 중...' : '나올 때까지 개봉'}</button>
            </div>
            {snipeResult && (
              <div className="mt-8 bg-gray-50 dark:bg-black/50 border border-gray-200 dark:border-white/10 rounded-xl p-6 flex flex-col items-center animate-fade-in-up transition-colors">
                <span className="text-rose-600 dark:text-rose-400 text-xs font-bold tracking-widest mb-4 transition-colors">TARGET OBTAINED</span>
                <div className="relative w-20 h-20 mb-4"><Image src={snipeResult.target.image} alt="T" fill unoptimized className="drop-shadow-sm dark:drop-shadow-none" /></div>
                <div className="text-xl font-black text-gray-900 dark:text-white text-center break-keep transition-colors">
      ...
      {showProbModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/40 dark:bg-black/80 backdrop-blur-sm animate-fade-in" onClick={() => setShowProbModal(false)}>
          <div className="bg-white dark:bg-[#111] border border-gray-200 dark:border-white/10 rounded-2xl p-6 max-w-sm w-full shadow-2xl transition-colors" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-6"><h3 className="text-lg font-black text-fuchsia-600 dark:text-fuchsia-400 transition-colors">{activeBox?.name}</h3><button onClick={() => setShowProbModal(false)} className="text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white transition-colors">✕</button></div>
            <div className="flex flex-col gap-2 max-h-[60vh] overflow-y-auto custom-scrollbar pr-2">
              {currentRewards.map((item, idx) => (
                <div key={idx} className="flex items-center gap-3 bg-gray-50 dark:bg-white/5 p-3 rounded-lg text-[11px] md:text-sm transition-colors">
                  ...
                  <span className="text-gray-700 dark:text-gray-200 flex-1 font-medium transition-colors">{item.name}</span>
                  <span className="text-white font-bold bg-fuchsia-100 dark:bg-fuchsia-500/20 text-fuchsia-600 dark:text-fuchsia-300 px-2 py-1 rounded transition-colors">{item.prob.toFixed(4)}%</span>
                </div>
              ))}
            </div>
          </div>
