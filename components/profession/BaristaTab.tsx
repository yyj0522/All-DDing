...
const CUSTOMERS = [
  { lv: 0, name: 'C급 샐리', id: 'npc_sally', grade: 'RARE', color: 'text-blue-400', border: 'border-blue-500/30', bg: 'bg-blue-500/10', glow: 'from-blue-500/20' },
  { lv: 1, name: 'B급 빈센트', id: 'npc_vincent', grade: 'EPIC', color: 'text-purple-400', border: 'border-purple-500/30', bg: 'bg-purple-500/10', glow: 'from-purple-500/20' },
  { lv: 2, name: 'A급 웬디', id: 'npc_wendy', grade: 'LEGENDARY', color: 'text-yellow-400', border: 'border-yellow-500/30', bg: 'bg-yellow-500/10', glow: 'from-yellow-500/20' },
  { lv: 3, name: 'S급 아이비', id: 'npc_ivy', grade: 'MYTHIC', color: 'text-red-400', border: 'border-red-500/30', bg: 'bg-red-500/10', glow: 'from-red-500/20' }
];
...
export default function BaristaTab() {
  ...
  const renderIngredient = (reqStr: string) => {
    ...
    if (match) {
      ...
      if (imgId) {
        return (
          <span key={reqStr} className="flex items-center gap-1.5 text-[10px] sm:text-xs bg-white/5 text-gray-300 px-2 py-1.5 rounded-lg border border-white/10">
            ...
            <span>{name} <span className="text-amber-400 font-bold">{count}</span></span>
          </span>
        );
      }
      return (
        <span key={reqStr} className="flex items-center gap-1 text-[10px] sm:text-xs bg-white/5 text-gray-400 px-2 py-1.5 rounded-lg border border-white/10">
          <span>{name} <span className="text-gray-300 font-bold">{count}</span></span>
        </span>
      );
    }
    return <span key={reqStr} className="text-[10px] sm:text-xs bg-white/5 text-gray-400 px-2 py-1.5 rounded-lg border border-white/5">{reqStr}</span>;
  };

  return (
    <div className="flex flex-col gap-8 w-full animate-fade-in-up mt-4">
        ...
        {CUSTOMERS.map((cust) => (
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 sm:px-5 sm:py-3 rounded-2xl font-bold transition-all duration-300 border ${
              selectedNpc === cust.lv
                ? `${cust.bg} ${cust.border} ${cust.color} shadow-[0_0_20px_rgba(0,0,0,0.5)] scale-105`
                : 'bg-white/5 border-white/10 text-gray-400 hover:text-gray-200 hover:bg-white/10'
            }`}
          >
            <div className="w-6 h-6 sm:w-8 sm:h-8 rounded-full overflow-hidden bg-black/50 border border-white/10 flex items-center justify-center p-0.5">
      ...
      <div className="space-y-6">
        {CUSTOMERS.filter(c => c.lv === selectedNpc).map((cust) => (
          <div key={cust.lv} className={`bg-[#0a0a0a] border ${cust.border} rounded-[2rem] flex flex-col shadow-2xl overflow-hidden relative`}>
            <div className={`absolute top-0 left-0 w-full h-48 sm:h-64 bg-gradient-to-b ${cust.glow} to-transparent opacity-40 pointer-events-none`}></div>
            
            <div className="flex flex-col sm:flex-row items-center justify-center sm:justify-start gap-4 sm:gap-6 py-4 px-6 sm:px-12 relative z-10 border-b border-white/5 bg-black/20">
              <div className="w-32 h-32 sm:w-40 sm:h-40 relative flex-shrink-0 drop-shadow-[0_15px_20px_rgba(0,0,0,0.8)]">
                <img 
              ...
              <div className="flex flex-col sm:flex-row items-center gap-3 sm:gap-4 text-center sm:text-left">
                <h4 className="text-3xl sm:text-5xl font-black text-white">{cust.name}</h4>
                <span className={`text-[10px] sm:text-xs font-black tracking-widest ${cust.color} ${cust.bg} px-4 py-1.5 sm:py-2 rounded-full border ${cust.border} whitespace-nowrap`}>
                  {cust.grade} 등급 전용 커피
                </span>
              </div>
            </div>

            <div className="p-6 md:p-10 bg-[#0a0a0a] relative z-10">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-6">
                {COFFEES.filter(c => c.reqLv === cust.lv).map(coffee => (
                  <div key={coffee.id} className="bg-[#111] rounded-2xl p-5 border border-white/5 hover:border-white/20 transition-colors group shadow-lg flex flex-col h-full">
                    <div className="flex items-start justify-between mb-5">
                      <div className="flex items-center gap-3">
                        <div className="w-12 h-12 bg-white/5 rounded-xl border border-white/10 p-2 flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform">
                        ...
                        <span className="text-lg font-bold text-gray-200 group-hover:text-white transition-colors">{coffee.name}</span>
                      </div>
                      <div className="flex flex-col items-end">
                        <span className="text-xs sm:text-sm font-black text-amber-500 bg-amber-500/10 px-2 sm:px-2.5 py-1 rounded-lg border border-amber-500/20 whitespace-nowrap">{coffee.price.toLocaleString()} G</span>
                      </div>
                    </div>
        ...
        <div className="bg-[#0a0a0a] border border-white/10 rounded-3xl p-8 shadow-2xl relative overflow-hidden">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4 relative z-10">
            {[
              ...
              return (
                <div key={idx} className="bg-[#111] border border-white/5 rounded-2xl p-4 flex flex-col items-center text-center hover:border-emerald-500/30 transition-all duration-300 group hover:bg-emerald-900/10 shadow-lg h-full">
                  {imgId && (
                    <div className="w-12 h-12 mb-3 bg-black/50 rounded-xl p-2 flex items-center justify-center border border-white/10 group-hover:scale-110 group-hover:border-emerald-500/50 transition-all shadow-inner flex-shrink-0">
                  ...
                  <span className="text-sm font-black text-emerald-400 mb-3 flex-grow">{mix.n}</span>
