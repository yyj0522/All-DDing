...
export default function Header() {
  ...
  return (
    <>
      <div className="fixed top-4 md:top-6 left-0 right-0 z-[60] flex justify-center px-4">
        <nav className="flex items-center justify-between w-full max-w-6xl px-6 py-4 bg-white dark:bg-black/80 border border-gray-200 dark:border-white/5 backdrop-blur-xl rounded-2xl md:rounded-full shadow-lg dark:shadow-[0_10px_30px_rgba(0,0,0,0.8)] transition-colors">
          <Link href="/" className="flex-shrink-0 md:mr-8 group relative z-[70]">
            <span 
              className="text-xl md:text-2xl font-black text-black dark:text-white tracking-widest transition-all duration-500 group-hover:tracking-[0.3em] drop-shadow-sm dark:drop-shadow-[0_0_10px_rgba(255,255,255,0.8)]" 
            >
              올띵
            </span>
          </Link>
          ...
          <div className="flex items-center gap-2 md:gap-4 relative z-[70]">
            ...
            <button
              ...
              className="p-2 rounded-full text-gray-900 dark:text-gray-400 hover:text-amber-500 dark:hover:text-amber-400 hover:bg-gray-100 dark:hover:bg-white/5 transition-all"
            ...
            <button 
              className="md:hidden text-gray-900 dark:text-gray-400 hover:text-black dark:hover:text-white p-2 transition-colors" 
      ...
      <div 
        className={`fixed inset-0 z-50 md:hidden bg-white/95 dark:bg-black/95 backdrop-blur-2xl transition-all duration-500 ease-in-out ${
      ...
      >
        <div className="flex flex-col items-center justify-center h-full gap-8 px-6 text-center">
          ...
          <Link 
            href="/settings"
            className={`text-gray-500 dark:text-gray-400 font-bold hover:text-gray-900 dark:hover:text-white transition-all ${isMenuOpen ? 'opacity-100' : 'opacity-0'}`}
