...
const MARKERS = [
  ...
  { id: 'orchard_coconut', name: '코코넛 과수원', x: -295, z: 275, realX: -295, realZ: 235, cat: '과수원' },
      ...
      <main className="relative z-10 flex-1 max-w-[1200px] w-full mx-auto px-4 pt-32 md:pt-40 pb-20 flex flex-col items-center">
        ...
        <div className="flex flex-col lg:flex-row gap-8 items-start justify-center w-full relative max-w-[1150px]">
          ...
          <div className="w-full lg:w-48 flex-shrink-0 flex lg:flex-col gap-2 overflow-x-auto custom-scrollbar pb-2 lg:pb-0">
            {(['전체', '농장', '광산', '과수원', '양식장', '서식지', '푸른석상', '하얀석상'] as Category[]).map((cat) => (
              <button
                ...
                className={`px-5 py-3.5 rounded-xl text-sm font-bold transition-all whitespace-nowrap text-left ${
                  activeCat === cat 
                  ? 'bg-white text-black shadow-[0_0_15px_rgba(255,255,255,0.2)]' 
                  : 'bg-[#0a0a0a] border border-white/5 text-gray-500 hover:text-white hover:border-white/20'
                }`}
              >
                {cat}
              </button>
          ...
          <div className="flex-1 w-full relative flex justify-center my-8 lg:my-0">
            <div 
              className={`relative w-full max-w-[950px] transition-all duration-300 ${mag.active ? 'cursor-auto' : 'cursor-crosshair hover:opacity-90'}`}
            ...
            >
              ...
              {mag.active && (
                <>
                  ...
                  <div 
                    className="absolute border-[3px] border-white/30 rounded-3xl shadow-[0_0_50px_rgba(0,0,0,0.8)] overflow-hidden z-50 bg-[#050505] pointer-events-none"
                    style={{
                      width: 360, 
                      height: 360,
                      left: mag.x, 
                      ...
                      boxShadow: '0 0 0 9999px rgba(0, 0, 0, 0.4), 0 25px 50px -12px rgba(0, 0, 0, 0.8)'
                    }}
                  >
                    <div 
                      className="absolute origin-top-left"
                      style={{
                        width: mag.mapW * ZOOM_LEVEL,
                        height: mag.mapH * ZOOM_LEVEL,
                        left: 180 - (mag.x * ZOOM_LEVEL),
                        top: 180 - (mag.y * ZOOM_LEVEL)
                      }}
                    >
