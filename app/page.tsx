      ...
      <main className="relative z-10 flex-1 max-w-5xl w-full mx-auto px-4 pt-32 md:pt-40 pb-20">
        <div className="mb-10 text-center md:text-left">
          <h1 className="text-4xl sm:text-5xl md:text-7xl font-black tracking-tighter mb-4 leading-tight">
            ...
            <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-cyan-400 drop-shadow-[0_0_15px_rgba(99,102,241,0.3)]">All-DDing</span>
          </h1>
          <p className="text-gray-400 max-w-2xl leading-relaxed text-sm md:text-base font-medium tracking-wide mx-auto md:mx-0">
            올띵은 띵타이쿤 유저들을 위한 <span className="font-bold text-red-400 underline underline-offset-4 decoration-red-500/50">비공식</span> 종합 도구 모음 사이트입니다.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 mb-16">
          <Link href="/efficiency" className="group col-span-1 md:col-span-3 relative rounded-3xl p-6 md:p-8 bg-gradient-to-br from-[#111] to-[#050505] border border-white/5 hover:border-indigo-500/30 transition-all duration-500 shadow-2xl overflow-hidden flex flex-col md:flex-row gap-8 items-center">
            <div className="w-full md:w-1/3 flex flex-col justify-center pr-4">
              ...
              <p className="text-gray-500 text-sm font-medium mb-6">개인 설정을 통해 나의 상황에 맞는 요리 효율을 확인해보세요.</p>
              <div className="inline-flex items-center gap-2 text-indigo-400 font-bold text-sm group-hover:translate-x-2 transition-transform">나의 음식 효율 보러가기</div>
            </div>
            <div className="w-full md:w-2/3 flex flex-col gap-3">
              {top3Recipes.map((item, index) => {
                const colors = [
                  { border: 'border-yellow-500/20', shadow: 'shadow-[0_0_15px_rgba(234,179,8,0.1)]', numBg: 'from-yellow-300 to-yellow-600', numText: 'text-black', percentText: 'text-yellow-400' },
                ...
                return (
                  <div key={item.name} className={`flex items-center justify-between p-3.5 md:p-4 rounded-2xl bg-white/[0.02] border ${colors.border} ${colors.shadow}`}>
                    <div className="flex items-center gap-4">
                      ...
                      <div className="w-10 h-10 bg-black/40 rounded-lg border border-white/10 flex items-center justify-center flex-shrink-0 overflow-hidden relative">
                        <Image src={`${STORAGE_BASE_URL}/foods/${ITEM_IMAGES[item.name]}.png`} alt={item.name} width={28} height={28} unoptimized priority className="object-contain" style={{ imageRendering: 'pixelated' }} />
                      </div>
                    ...
                    <div className="text-right flex flex-col items-end pl-4">
                      <div className={`font-black whitespace-nowrap ${index === 0 ? 'text-xl' : 'text-lg'} ${colors.percentText}`}>{item.currentMarketPrice.toLocaleString()} G</div>
                      <div className="text-[11px] text-gray-400 font-bold whitespace-nowrap mt-0.5">최고가 대비 <span className={item.percentToMax >= 80 ? 'text-emerald-400' : 'text-white'}>{item.percentToMax.toFixed(1)}%</span></div>
                    </div>
                  </div>
                );
              })}
          ...
          <Link href="/profession" className="group col-span-1 rounded-3xl p-6 md:p-8 bg-[#0a0a0a] border border-white/5 hover:border-amber-500/30 transition-all flex flex-col justify-between min-h-[200px]">
             ...
             <div><h3 className="text-xl font-bold text-white mb-2">전문가</h3><p className="text-gray-500 text-sm leading-relaxed">각 전문가별 제작법/계산기를 제공합니다.</p></div>
          </Link>
          <Link href="/map" className="group rounded-3xl p-6 md:p-8 bg-[#0a0a0a] border border-white/5 hover:border-emerald-500/30 transition-all flex flex-col justify-between min-h-[200px]">
            ...
            <div><h3 className="text-xl font-bold text-white mb-2">아일랜드 지도</h3><p className="text-gray-500 text-sm leading-relaxed">아일랜드를 한 눈에 파악해보세요.</p></div>
          </Link>
          <Link href="/resources" className="group rounded-3xl p-6 md:p-8 bg-[#0a0a0a] border border-white/5 hover:border-cyan-500/30 transition-all flex flex-col justify-between min-h-[200px]">
            ...
            <div><h3 className="text-xl font-bold text-white mb-2">자료실</h3><p className="text-gray-500 text-sm leading-relaxed">띵타이쿤 플레이에 도움이되는 모드를 다운받을 수 있습니다.</p></div>
          </Link>
        </div>

        <section className="bg-[#0a0a0a] border border-white/10 rounded-3xl p-8 shadow-2xl">
          <div className="flex items-center justify-between mb-6 border-b border-white/5 pb-4">
            <div className="flex items-center gap-3"><svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9M7 16h6M7 8h6v4H7V8z" /></svg><h2 className="text-xl font-bold text-white">올띵 업데이트</h2></div>
            <Link href="/note" className="text-gray-400 hover:text-white transition-colors flex items-center gap-1 text-sm font-medium group">전체보기<svg className="w-4 h-4 group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg></Link>
          </div>
          <div className="space-y-4">
            {releaseNotes.map(note => (
              <div key={note.id} className="flex flex-col md:flex-row md:items-center gap-4">
                ...
                <span className="text-gray-600 text-xs md:ml-auto font-mono">{new Date(note.created_at).toISOString().split('T')[0]}</span>
              </div>
            ))}
