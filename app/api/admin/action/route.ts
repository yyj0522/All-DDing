...
export async function POST(request: Request) {
  try {
    ...
    if (action === 'create_note') {
      const { error } = await supabaseAdmin.from('release_notes').insert([payload]);
      if (error) throw error;
      return NextResponse.json({ success: true });
    }

    if (action === 'update_note') {
      const { error } = await supabaseAdmin.from('release_notes').update(payload).eq('id', id);
      if (error) throw error;
      return NextResponse.json({ success: true });
    }

    if (action === 'delete_note') {
      const { error } = await supabaseAdmin.from('release_notes').delete().eq('id', id);
      if (error) throw error;
      return NextResponse.json({ success: true });
    }
